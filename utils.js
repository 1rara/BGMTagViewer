...
function tozh_s(cc) {
  ...
  for (let i = 0; i < cc.length; i++) {
    if (zh_t.indexOf(cc.charAt(i)) != -1)
      str += zh_s.charAt(zh_t.indexOf(cc.charAt(i)));
    else
      str += cc.charAt(i);
  }
  return str;
...
function formatTag(str) {
  //合并繁简体、大小写、空格、以及分隔符
  return tozh_s(str.replaceAll(/(\\t)|[\s、・,，·/.。\-—_]/g, '')).toLowerCase();
}

function bindName(arr, sName) {
  if (!bind) return sName;
  let start = 0;
  let end = arr.length - 1;
  let target = formatTag(sName);
  ...
  while (start <= end) {
    ...
    if (arr[middle][0] === target) {
      return arr[middle][1];
    } else if (arr[middle][0].localeCompare(target) < 0) {
      start = middle + 1;
    } else {
      end = middle - 1;
    }
