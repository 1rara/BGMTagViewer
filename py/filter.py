...
with jsonlines.open('./data/person.jsonlines') as jl:
    for line in jl:
        m=re.search(r'中文名= (.+?)\r', line['infobox']) #简体中文名
        if m:
            key=formatTag(m.group(1))
            altRaw.add((formatTag(line['name']), key)) #人物条目名
            m=re.search(r'别名=\{(.+?)\}', line['infobox'], flags=re.S) #别名
            if m:
                for i in re.findall(r'\[(?:[^\]]+?\|)?(.*?)\]', m.group(1)):
...
with jsonlines.open('./data/subject_cleaned.jsonlines') as jl:
    for i in jl:
        s=i['year']
        for t in i['tags']:
            name=t['name']
